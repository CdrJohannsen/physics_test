...
Vect operator*(float a, Vect b) { return b * a; }
...
void calcSpeed(Ball& ball1, Ball& ball2, Vect axis) {
    ball1.velocity =
        axis * calcSpeedAxis(ball1.velocity.length(), ball2.velocity.length(), ball1.w, ball2.w) * friction;
    ball2.velocity =
        axis * -calcSpeedAxis(ball2.velocity.length(), ball1.velocity.length(), ball2.w, ball1.w) * friction;
}
...
void updatePhysics(Ball& ball, Ball* balls, int index, int ball_count) {
    ...
    for (int i = 0; i < ball_count; i++) {
        ...
        Ball ball2 = balls[i];
        Vect col = ball.position - ball2.position;
        float d = sqrt(pow(col.x, 2) + pow(col.y, 2));
        float min_dist = (ball.r + ball2.r);
        if (d < min_dist - 1) {
            Vect axis = col / d;
            ball.position += axis * (min_dist - d);
            // ball.velocity.x = (ball.velocity.x * (ball.r - balls[i].r) + (2 * balls[i].r * balls[i].velocity.x)) /
            // (ball.r
            ...
            // ball.r) + (2 * ball.r * ball.velocity.x)) / (balls[i].r + ball.r); balls[i].velocity.y =
            // (balls[i].velocity.y * (balls[i].r - ball.r) + (2 * ball.r * ball.velocity.y)) / (balls[i].r + ball.r);
